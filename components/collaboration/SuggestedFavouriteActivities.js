...
const SuggestedFavouriteActivities = ({ activities, handleFavouritesSelect }) => {
	const renderFavourites = (activity, selected, index) => {
		// console.log("bad coding1", activity);
		...
		return (
			<View style={styles.card}>
				<Badge
					status='success'
					value={selected ? (activity.votes + 1).toString() : activity.votes.toString()}
					textStyle={{ color: 'white' }}
					badgeStyle={
						selected ? { backgroundColor: '#E86830' } : { backgroundColor: 'black' }
					}
				...
				<View style={{ flex: 3, marginTop: 5 }}>
					...
					<View style={{ justifyContent: 'flex-end', flex: 1 }}>
						...
						>
							{selected ? (
								<View>
									<Text style={styles.favouritesButtonText}>UNVOTE</Text>
								</View>
							) : (
	...
	return (
		<View style={{ marginTop: 10 }}>
			<Text style={styles.genreSelectionText}>Suggested activities:</Text>
			{activities.length == 0 ? (
				<View
					style={{
						justifyContent: 'center',

...
const AllPlans = ({
				...
				if (allUsers[userID].hasOwnProperty('collab_boards')) {
					...
					for (var board in allCollaborations) {
						...
						collabBoard.isUserHost =
							boardID.substring(0, boardID.indexOf('_')) == userID;
	...
	const renderAppropriateScreen = () => {
		var isAddingFavourite = route.params == undefined ? false : route.params.addingFavourite; // Undefined if come from Feed using bottom tab nav
					...
					<View style={{ flex: 9 }}>
						<View style={{ flex: 5, justifyContent: 'center' }}>
							<View style={{ height: '75%', width: '60%', alignSelf: 'center' }}>
								<Image
									style={styles.image}
									source={require('../../assets/EmptyCollaborations.png')}
								/>
							</View>
						</View>
						<View style={{ flex: 1 }}>
							...
							>
								Add some friends in DoWhat and plan with them! Your plans with your
								DoWhat friends will appear here.
							</Text>
						</View>

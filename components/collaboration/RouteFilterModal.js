...
const RouteFilterModal = ({ onClose, navigation }) => {
	const routeOptions = ['invite', /*'collab',*/ 'manual']; // Collab route omitted for Beta release
	...
	const renderInviteCard = () => {
		return (
			<TouchableOpacity
				style={[styles.individualCard, { backgroundColor: '#5846DE' }]}
	...
	const renderManualCard = () => {
		return (
			<TouchableOpacity
				style={[styles.individualCard, { backgroundColor: '#B65E76' }]}
	...
	const renderCollabCard = () => {
		return (
			<TouchableOpacity
				style={[styles.individualCard, { backgroundColor: '#E29E40' }]}
		...
		<View style={styles.modalContainer}>
			...
			<View style={{ flex: 1 }}>
				<FlatList
					data={routeOptions}
					horizontal={true}
					renderItem={({ item }) => renderIndividualOptions(item)}
